...
- **Strict Invariant: Zero LLM / Pure Dumb I/O**: The background daemon in `.claude/skills/telegram/daemon/telegram_daemon.py` contains zero LLM inference, zero synthetic rule heuristics, and zero pseudo-brain logic. It functions strictly as a deterministic network I/O, event queue, and HTTP REST gateway.
- **Port Allocation**:
  - `.claude/skills/sound/daemon/audio_daemon.py`: Acoustic Core TTS on `http://127.0.0.1:20129`.
  - `.claude/skills/telegram/daemon/telegram_daemon.py`: Telegram C2 Gateway on `http://127.0.0.1:20130`.
- **Master Configuration**: Parameters are resolved from `faust_config.json` (`telegram` block) with priority cascade.
- **Single-Instance Mutex Lock**: Atomic file creation with OS process handle inspection in `.claude/skills/telegram/daemon/telegram_daemon.lock` preventing HTTP 409 Conflict.
