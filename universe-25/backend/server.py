...
def load_global_spec() -> Dict[str, Any]:
    if not SPEC_PATH.exists():
        raise HTTPException(status_code=500, detail=f"Missing api_spec.json at: {SPEC_PATH}")
...
def load_project_index(project_id: str) -> Dict[str, Any]:
    ...
    if not index_path.exists():
        raise HTTPException(status_code=404, detail=f"Project '{project_id}' does not exist at: {index_path}")
...
async def execute_order(req: OrderRequest):
    ...
    if act not in allowed_actions and act not in extended_actions:
        err = f"Format '{fmt}' for Tier {tier} does not support action '{act}'."
        if log_enabled: write_error_log(project_id, err)
        raise HTTPException(status_code=400, detail=err)
    ...
    if act == "backup":
        if not file_path.exists(): return {"status": "skipped", "message": "File does not exist yet"}
    ...
    if fmt == "json":
        ...
        if act == "append":
            ...
            return {"status": "success", "message": "Cleared JSONL buffer"}
        ...
        with sqlite3.connect(file_path) as conn:
            ...
            if act == "read_all":
                ...
                if not isinstance(payload, dict):
                    raise ValueError("Action 'overwrite_all' requires Dictionary payload")

                if not payload:
                    return {"status": "skipped", "message": "Empty payload, safe abort."}
